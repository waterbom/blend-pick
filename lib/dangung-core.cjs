...
const {enqueue}=require('./dangung-notifications.cjs');
...
function service(pool,provider,{now=()=>new Date()}={}){
 ...
 async function owned(c,id,owner){if(typeof id!=='string'||!/^[a-f0-9-]{36}$/i.test(id))fail('예약을 찾을 수 없습니다.',404);const {rows:[r]}=await c.query('SELECT * FROM dangung_reservations WHERE id=$1',[id]);if(!r)fail('예약을 찾을 수 없습니다.',404);
 if(owner!==null&&r.owner_hash!==hash(owner)){
 const {rows:[access]}=await c.query('SELECT 1 FROM dangung_lookup_access WHERE reservation_id=$1 AND owner_hash=$2 AND expires_at>$3',[id,hash(owner),now()]);
 if(!access)fail('예약을 찾을 수 없습니다.',404);
 }return r;}
 async function lookup(input,owner){
 const id=typeof input.orderId==='string'?input.orderId.trim().toLowerCase():'',phone=typeof input.phone==='string'?input.phone.replace(/[^0-9]/g,''):'';
 const missing=()=>fail('예약번호와 휴대폰번호를 다시 확인해 주세요.',404);
 if(!/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/.test(id)||!/^01[016789]\d{7,8}$/.test(phone))return missing();
 // Persist attempts outside the lookup transaction so a failed lookup cannot roll them back.
 for(const key of ['reservation:'+id,'browser:'+hash(owner)]){
 const {rows:[limit]}=await pool.query(`INSERT INTO dangung_lookup_attempts(key_hash,attempts,resets_at) VALUES($1,1,$3)
 ON CONFLICT(key_hash) DO UPDATE SET attempts=CASE WHEN dangung_lookup_attempts.resets_at<=$2 THEN 1 ELSE dangung_lookup_attempts.attempts+1 END,
 resets_at=CASE WHEN dangung_lookup_attempts.resets_at<=$2 THEN $3 ELSE dangung_lookup_attempts.resets_at END RETURNING attempts`,[hash(key),now(),new Date(now().getTime()+15*60000)]);
 if(limit.attempts>10)fail('조회 시도가 많습니다. 15분 후 다시 시도해 주세요.',429);
 }
 return tx(async c=>{await expire(c);const {rows:[r]}=await c.query('SELECT * FROM dangung_reservations WHERE id=$1 AND buyer_phone=$2',[id,phone]);
 if(!r)return missing();
 await c.query(`INSERT INTO dangung_lookup_access(reservation_id,owner_hash,expires_at) VALUES($1,$2,$3)
 ON CONFLICT(reservation_id,owner_hash) DO UPDATE SET expires_at=EXCLUDED.expires_at`,[id,hash(owner),new Date(now().getTime()+60*60000)]);
 ...
 await c.query("UPDATE dangung_reservations SET status='paid',payment_key=$2,receipt_url=$3,paid_at=$4,updated_at=$4 WHERE id=$1",[id,payment.paymentKey,receipt,now()]);await audit(c,'paid',id);await enqueue(c,await owned(c,id,null),'confirmed');
 ...
 await c.query("UPDATE dangung_reservations SET status='cancelled',updated_at=$2 WHERE id=$1",[id,now()]);await c.query('DELETE FROM dangung_occupancy WHERE reservation_id=$1',[id]);await audit(c,'cancelled',id,{refundAmount:refunded});await enqueue(c,await owned(c,id,null),'cancelled');
 }
 ...
 async function requestCancel(id,owner){return tx(async c=>{const r=await owned(c,id,owner);if(r.status!=='paid')fail('결제 완료 예약만 취소를 요청할 수 있습니다.',409);if(r.cancel_requested)return publicReservation(r);await c.query('UPDATE dangung_reservations SET cancel_requested=true WHERE id=$1',[id]);await audit(c,'cancel_requested',id);return publicReservation(await owned(c,id,owner));});}
 ...
 if(amount===0)return tx(async c=>{await c.query("UPDATE dangung_reservations SET status='cancelled',updated_at=$2 WHERE id=$1 AND status='cancelling' AND refund_amount=0",[id,now()]);await c.query('DELETE FROM dangung_occupancy WHERE reservation_id=$1',[id]);await audit(c,'cancelled_without_refund',id);await enqueue(c,await owned(c,id,null),'cancelled');return publicReservation(await owned(c,id,null));});
 return finish(id,await provider.cancel(r.payment_key,amount,reason,id));
 }
 async function admin(){const calendarData=await calendar();const {rows}=await pool.query('SELECT id,status,check_in,check_out,guests,buyer_name,buyer_phone,memo,amount,quote,refund_amount,cancel_requested,created_at FROM dangung_reservations ORDER BY created_at DESC LIMIT 300');const {rows:[totals]}=await pool.query("SELECT count(*) FILTER(WHERE status='paid')::int AS paid_count,COALESCE(sum(amount) FILTER(WHERE status IN ('paid','cancelling')),0)::bigint AS paid_amount,COALESCE(sum(amount-refund_amount) FILTER(WHERE status='cancelled'),0)::bigint AS retained_amount FROM dangung_reservations");const {rows:notifications}=await pool.query('SELECT id,reservation_id,kind,status,attempts,last_error,updated_at FROM dangung_notifications ORDER BY updated_at DESC LIMIT 100');return {...calendarData,reservations:rows,totals,notifications};}
 ...
 return {calendar,getQuote,reserve,lookup,status,confirm,abandon,requestCancel,reconcile,cancel,admin,settings,setDates};
}
