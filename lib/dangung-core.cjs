...
function validateConfig(c){
 ...
 if(c.enabled){integer(c.extraGuestFee,0,1000000,'추가 인원 요금');integer(c.bbqFee,0,1000000,'바비큐 요금');integer(c.depositAmount,0,1000000,'현장 보증금');
...
function quote(config,version,rows,input,now=new Date()){
 ...
 integer(input.guests,1,16,'이용 인원');if(typeof input.bbq!=='boolean')fail('바비큐 선택을 확인해 주세요.');
 ...
 const lodging=nightly.reduce((s,d)=>s+d.price,0), extra=Math.max(0,input.guests-6)*config.extraGuestFee*(config.extraGuestUnit==='perNight'?stay.length:1),bbq=input.bbq?config.bbqFee:0;
 return {version,checkIn:input.checkIn,checkOut:input.checkOut,guests:input.guests,nightly,lodging,extra,bbq,total:lodging+extra+bbq,extraGuestUnit:config.extraGuestUnit,depositAmount:config.depositAmount,depositTerms:config.depositTerms,refundTerms:config.refundTerms};
}
const publicReservation=r=>({id:r.id,status:r.status,checkIn:iso(r.check_in),checkOut:iso(r.check_out),guests:r.guests,buyerName:r.buyer_name,quote:r.quote,amount:r.amount,expiresAt:r.expires_at,receiptUrl:r.receipt_url,cancelRequested:r.cancel_requested,refundAmount:r.refund_amount});
function service(pool,provider,{now=()=>new Date()}={}){
 ...
 if(prior){if(iso(prior.check_in)!==input.checkIn||iso(prior.check_out)!==input.checkOut||prior.guests!==input.guests||prior.amount!==input.amount||prior.buyer_name!==input.buyerName?.trim()||prior.buyer_phone!==input.buyerPhone||prior.memo!==input.memo)fail('이미 사용한 예약 요청입니다. 새로 확인해 주세요.',409);return publicReservation(prior);}
