import { bookingService as service,guard,body,failure,paymentReady } from '@/lib/dangung-server';
import { BookingError } from '@/lib/dangung-core.cjs';
export const runtime='nodejs';
export const dynamic='force-dynamic';
export async function GET(){try{await guard(undefined,true);return Response.json({...await service.admin(),paymentReady:paymentReady()},{headers:{'Cache-Control':'no-store'}});}catch(e){return failure(e);}}
export async function POST(request:Request){try{await guard(request,true);const input=await body(request);let result;
 ...
 }return Response.json(result,{headers:{'Cache-Control':'no-store'}});}catch(e){return failure(e);}}
