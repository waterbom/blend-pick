...
| `app/sanji/` | 산지픽 페이지 | 루트(`/sanji`) = 대표 상품 **판매 페이지** · 개별 상품(`/sanji/p/<id>`) · 브랜드 소개(`/sanji/about`, 예전 랜딩) |
...
- 산지픽에 노출할 상품 = 상품 관리에서 **카테고리 `산지픽`** 으로 지정한 판매 중 상품.
  - 루트 `/` 는 그중 **가장 최근 등록된 판매중(재고 있음) 상품**을 곧바로 판매 페이지로 띄운다. 나머지는 "다른 고객이 함께 본 상품"에 깔리고 `/p/<id>` 로 각각 열린다.
  - 산지픽 상품이 하나도 없으면 예시 화면(구매 잠김)이 보인다.
