...
## 제품 사진 (`public/sanji/products/<품목>/`)

감자(7) · 복숭아(7) · 단호박(5) · 배도라지(3) · 사과(7), 1000×1000 webp. 상품 관리에서 상품을 등록할 때
대표 이미지·추가 이미지 URL로 그대로 쓸 수 있다 (예: `/sanji/products/감자/감자.webp`).
상품이 등록되기 전까지는 예시 상품(demo)이 이 사진으로 채워진다.
